...
func _init() -> void:
  ...
  for i: int in range(0, 3):
    ...
    var params := Taskoid.Params.new(task_names[i], task_names[i], difficulties[i], parent, false, "", null)
  ...
  for i: int in task_names.size():
    ...
    var params := Taskoid.Params.new(task_names[i], task_names[i], difficulties[difficulty_index], test_projects[parent_index], false, "", null)
    test_tasks.append(
      Task.new(params))
